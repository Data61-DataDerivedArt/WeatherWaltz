...
import os
from . import common
from common import ominosity
from . import dicemidi
instrument = dicemidi.instrument

################################################################

def ominise(music, bar):
    if ominosity[bar] <= 3:
        return " << { %s } \\\\ \\transpose c' bes { %s } \\\\ \\transpose c' aes { %s } >> " % (music, music, music)
    if ominosity[bar] <= 6:
        return " << { %s } \\\\ \\transpose c' aes { %s }>> " % (music, music)
    return music

# lilypond engraver
def out(dicelist, ext='.ly', papersize='a4', landscape=False, signature='signature.eps', bass=0, treble=0, tempo=150, title=''):
    print ominosity
    ...
    rh[4] = "g''8 d''4\\trill"
    lh[4] = "g,16 b, g8 b,"
    rhEndingOne = " <g'' b' d'' g''>4 r8 "
    rhEndingTwo = " g'8 b16 g fis e "
    ...
    lhEndingOne = '{g8 g16 f! e d}'
    lhEndingTwo = '{g8 b16 g fis e}'
    rh[6] = "g'8 c'' e''"
    rh[7] = "e''16 c'' e'' g'' c''' g''"
    lh[7] = "<c g>4 r8"
...
#    s.append('    }\n')
    ...
    s.append('    \\DiceWaltz\n')
    s.append('  >>\n')
    s.append('  \\midi {\\tempo 8=' + '{}'.format(tempo) + '}\n')
    s.append('}\n')
    s.append('\n')
    return ''.join (s)
